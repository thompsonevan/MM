...
public class DriveTrain implements IHotSensedActuator<RobotState, RobotCommandProvider, Integer > {
    ...
    public void performAction(RobotCommandProvider commander, RobotState robotState) {
        ...
        if (commander.getAimingEnabled()){
            ...
            double power = commander.getPowerState();

            driveRightLeader.set(ControlMode.PercentOutput, ((commander.getDriveCommand() + commander.getTurnCommand()) * power) + correction);
            driveLeftLeader.set(ControlMode.PercentOutput, ((commander.getDriveCommand() - commander.getTurnCommand()) * power) - correction);
